...
perf <- function(){
    ...
    res <- microbenchmark(randmatstat(1000), times=21L, unit="ns")
    tmed_randmatstat <- median(res$time)
    tmed_randmatstat <- tmed_randmatstat * conv_nanosecond_to_second
    cat("median time is (randmatstat): ", tmed_randmatstat, " seconds \n", sep="")

    tmed <- c(tmed_pisum, tmed_fib, tmed_randmatstat)
    algoNames <- c("pisum", "fib", "randmatstat")
